...
skillCreatorRouter.post('/', ensureLlmApiKey, ensureUserCredentials, async (req, res) => {
  ...
  if (!description || typeof description !== 'string' || description.trim().length < 5) {
    return logAndRespond(res, 400, { error: 'description is required (min 5 characters)' }, ENDPOINT)
  }
  if (host && !VALID_HOSTS.includes(host)) {
    return logAndRespond(res, 400, { error: `host must be one of: ${VALID_HOSTS.join(', ')}` }, ENDPOINT)
  }
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      parsed.description = typeof parsed.description === 'string' ? parsed.description : parsed.name
      ...
      lastError = err
      req.logger.warn(`${ENDPOINT} attempt ${attempt}/2 failed`, { error: err.message, traffic: 'system' })
    }
  }

  req.logger.error(`${ENDPOINT} failed after 2 attempts`, { error: lastError?.message, traffic: 'system' })
  return logAndRespond(res, 500, { error: 'Failed to generate skill. Please try again.' }, ENDPOINT)
})
