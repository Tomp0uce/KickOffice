...
uploadRouter.post('/', upload.single('file'), async (req, res) => {
  if (!req.file) {
    return logAndRespond(
      res,
      400,
      { code: ErrorCodes.NO_FILE_UPLOADED, error: 'No file uploaded' },
      'POST /api/upload',
    );
  }
  ...
  try {
    ...
    // DOCX Extraction
      ...
      try {
        ...
        return logAndRespond(
          ...
          400,
          {
            code: ErrorCodes.DOCX_EXTRACTION_FAILED,
      ...
      for (const worksheet of workbook.worksheets) {
        ...
        worksheet.eachRow(row => {
          const cells = row.values.slice(1).map(v => {
            if (v == null) return '';
            const raw = v instanceof Object && 'result' in v ? (v.result ?? '') : v;
            const s = String(raw);
    ...
    // Image types — return base64 data-URI for LLM vision
      ...
      req.logger.info(`POST /api/upload completed image encoding`, {
        filename,
        bytes: file.size,
        imageId,
      });
    ...
    if (!extractedText || !extractedText.trim()) {
      return logAndRespond(
        ...
        400,
        {
          ...
          error:
            'No text could be extracted from the file. Make sure the file contains readable text.',
        },
        'POST /api/upload',
      );
    }
    ...
    if (extractedText.length > TEXT_MAX_CHARS) {
      extractedText =
        extractedText.substring(0, TEXT_MAX_CHARS) + '\n\n... [Content truncated due to file size]';
    }
    ...
    req.logger.info(`POST /api/upload completed file parsing`, {
      filename,
    ...
    return logAndRespond(
      res,
      500,
      { code: ErrorCodes.INTERNAL_ERROR, error: 'Failed to process file' },
      'POST /api/upload',
    );
  }
});
