...
export function useAgentLoop(options: UseAgentLoopOptions) {
  ...
  function buildToolSignature(allToolNames: string[]): string {
    return allToolNames.slice().sort().join('|')
  }

  function persistEnabledTools(allToolNames: string[], enabledToolNames: Set<string>): void {
  ...
  function getEnabledToolNamesFromStorage(allToolNames: string[]): Set<string> {
    ...
    try {
      ...
      const enabledTools = new Set<string>(parsed.enabledToolNames.filter((name: unknown): name is string => typeof name === 'string' && allToolNameSet.has(name)))
