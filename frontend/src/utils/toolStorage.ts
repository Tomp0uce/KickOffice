 ...
 */
function migrateToolPreferences(storedEnabledNames: string[], allToolNames: string[]): Set<string> {
...
export function getEnabledToolNamesFromStorage(allToolNames: string[]): Set<string> {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const legacyEnabledTools = new Set<string>(
        parsed.filter(
          (name): name is string => typeof name === 'string' && allToolNameSet.has(name),
        ),
      );
    ...
    const enabledTools = new Set<string>(
      parsed.enabledToolNames.filter(
        (name: unknown): name is string => typeof name === 'string' && allToolNameSet.has(name),
      ),
    );
