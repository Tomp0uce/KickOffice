...
export async function insertMarkdownIntoTextRange(context: any, textRange: any, text: string, forceStripBullets = false) {
  ...
  insertImageOnSlide: {
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ...
      if (!base64 || base64.length < 10) {
        throw new Error('Error: base64Data is empty or not a valid image. If you uploaded a file, use the image ID shown in the upload confirmation.')
      }
...
5. Slide numbers are 1-based in UI, 0-indexed in arrays`,
  ...
  insertContent: {
    name: 'insertContent',
    category: 'write',
    description: 'The PREFERRED tool for adding or replacing content in PowerPoint. Supports Markdown (bold, italic, bullets). Requires BOTH slideNumber and shapeIdOrName to target a specific shape (ALWAYS use getShapes first to discover IDs). Without shapeIdOrName it only replaces the currently selected text — never use it on an empty slide without providing shapeIdOrName or you will create a floating text box instead of filling the placeholder.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
        ...
        shapeIdOrName: {
          type: 'string',
          description: 'ID or Name of the shape to update (from getShapes). ALWAYS provide this to fill a specific placeholder. If omitted, replaces current text selection only.',
        },
      },
      required: ['content'],
    },
  ...
  addSlide: {
    name: 'addSlide',
    category: 'write',
    description: "Add a new slide to the presentation. The tool automatically picks the best matching layout from the presentation's slide master. Pass title and body to automatically populate the template text boxes (recommended).",
    inputSchema: {
      type: 'object',
      properties: {
        layout: {
          type: 'string',
          description: "Hint for the desired layout type: 'TitleAndContent' for standard content slides with bullet points (DEFAULT), 'Title' for a title-only or title-slide layout, 'Blank' for an empty slide.",
        },
        title: {
          type: 'string',
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ...
      const slides = context.presentation.slides
      const layoutHint = typeof args.layout === 'string' ? args.layout.trim().toLowerCase() : 'titleandcontent'
      ...
      try {
        ...
        if (slideMasters.items.length > 0) {
          ...
          const scoreLayout = (name: string): number => {
            ...
            if (layoutHint.includes('blank') || layoutHint === 'vide' || layoutHint === 'vierge') {
            ...
            if (layoutHint === 'title' || layoutHint === 'titre') {
              ...
              if (n.includes('title') && !n.includes('content') && !n.includes('contenu') && !n.includes('body') && !n.includes('texte') && !n.includes('media') && !n.includes('picture')) return 50
              return 0
            }
            // Default: TitleAndContent / content slides
            if (n.includes('title and content') || n.includes('titre et contenu')) return 100
            if (n.includes('title') && (n.includes('content') || n.includes('contenu') || n.includes('text') || n.includes('texte'))) return 90
            ...
            if (n.includes('blank') || n.includes('vide') || n === 'title' || n === 'titre') return 0
          ...
          for (const lo of availableLayouts) {
            ...
            if (s > bestScore) { bestScore = s; bestLayout = lo }
          }
  ...
  screenshotSlide: {
    ...
    category: 'read',
    description: 'Capture a visual screenshot of a PowerPoint slide as PNG image. Use after making changes to verify visual rendering. Requires PowerPointApi 1.5+.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ...
      const index = Math.trunc(slideNumber) - 1
      if (index < 0 || index >= slides.items.length) throw new Error(`Slide ${slideNumber} does not exist.`)
      const slide = slides.getItemAt(index)
      const imageResult = (slide as any).getImageAsBase64({ width: 960 })
      await context.sync()
      const base64 = imageResult.value as string
      return JSON.stringify({ __screenshot__: true, base64, mimeType: 'image/png', description: `Screenshot of slide ${slideNumber}` })
    },
  },
  ...
  duplicateSlide: {
    name: 'duplicateSlide',
    category: 'write',
    description: 'Duplicate a slide by its 1-based index. The copy is inserted after the original.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ensurePowerPointRunAvailable()
      const slideNumber = Number(args.slideNumber)
      if (!Number.isFinite(slideNumber) || slideNumber < 1) throw new Error('slideNumber must be >= 1.')
      const slides = context.presentation.slides
      slides.load('items')
      await context.sync()
      const index = Math.trunc(slideNumber) - 1
  ...
  verifySlides: {
    name: 'verifySlides',
    category: 'read',
    description: 'Verify slide layouts by checking for shape overflows and overlaps across all slides. Returns a report of detected issues.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
      ...
      for (let i = 0; i < slides.items.length; i++) {
        ...
        for (const s of shapes) {
          if (s.left + s.width > slideWidth + 1 || s.top + s.height > slideHeight + 1) {
            issues.push(`Slide ${i + 1}: shape "${s.name}" overflows slide boundaries (right=${Math.round(s.left + s.width)}, bottom=${Math.round(s.top + s.height)})`)
          }
        }
...
ALWAYS call markDirty() after modifying the zip.`,
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ...
      const result = await withSlideZip(context, slideIndex, async (zip: any, markDirty: () => void) => {
  ...
  searchIcons: {
    name: 'searchIcons',
    category: 'read',
    description: 'Search for icons by keyword using the Iconify library (thousands of icon sets including Material Design, Fluent, Feather). Returns icon IDs in format "prefix:name".',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search keyword (e.g. "home", "user", "chart").' },
        limit: { type: 'number', description: 'Max results. Default: 10.' },
        prefix: { type: 'string', description: 'Filter by icon set prefix (e.g. "mdi" for Material Design, "fluent" for Fluent UI).' },
      },
      required: ['query'],
    },
    executeCommon: async (args: Record<string, any>) => {
  ...
  insertIcon: {
    name: 'insertIcon',
    category: 'write',
    description: 'Insert an icon from Iconify into a slide. Find icon IDs first using searchIcons. Icon is inserted as an SVG image.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        color: { type: 'string', description: 'Icon color as hex, e.g. "#FF5733". Uses black if omitted.' },
      },
    ...
    executePowerPoint: async (context: any, args: Record<string, any>) => {
      ...
      if (parts.length !== 2) throw new Error('iconId must be in format "prefix:name", e.g. "mdi:home".')
