  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timeoutId = setTimeout(() => {
        logService.warn(`[OfficeAction] Timeout executing ${actionName} (attempt ${attempt + 1})`, { actionName })
    ...
    try {
      ...
      const isBusy = err instanceof Error && (err.message === OFFICE_BUSY_TIMEOUT_MESSAGE || err.message.includes('busy'))
      
      const shouldRetry = (isGeneralException || isBusy) && attempt < maxRetries
      
      if (!shouldRetry) {
        if (err instanceof Error && err.message !== OFFICE_BUSY_TIMEOUT_MESSAGE && err.message !== 'Operation aborted by user') {
          logService.error(`[OfficeAction] Error executing ${actionName}`, err, { actionName })
        }
        throw err
      }
      
      logService.warn(`[OfficeAction] Retrying ${actionName} after error: ${err instanceof Error ? err.message : String(err)}. Attempt ${attempt + 1} of ${maxRetries}`, { actionName })
