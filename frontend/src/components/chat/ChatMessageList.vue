      ...
      <div class="flex min-w-0 flex-1 flex-col gap-1 items-start text-left">
        ...
        >
          <div class="flex items-start gap-2 text-accent" role="status" aria-live="polite">
            <span
              class="inline-flex mt-0.5 h-2 w-2 shrink-0 animate-pulse rounded-full bg-accent"
            />
...
const historyWithSegments = computed(
  () => props.historyWithSegments ?? context.historyWithSegments.value,
);
...
const backendOfflineLabel = computed(
  () => props.backendOfflineLabel ?? context.t('backendOffline'),
);
const replaceSelectedText = computed(
  () => props.replaceSelectedText ?? context.t('replaceSelectedText'),
);
...
const thoughtProcessLabel = computed(
  () => props.thoughtProcessLabel ?? context.t('thoughtProcess'),
);
