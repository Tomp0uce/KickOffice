<template>
  ...
  >
    ...
    <div class="relative flex items-center" ref="dropdownRef">
      <button
        type="button"
        class="flex items-center gap-1 px-2 py-1 rounded text-xs text-secondary hover:bg-bg-tertiary transition-colors max-w-[120px]"
        :disabled="loading"
      ...
      >
        ...
        <ChevronDown :size="11" :class="dropdownOpen ? 'rotate-180' : ''" class="shrink-0 transition-transform" />
      </button>
      ...
      >
        ...
        <button
          ...
          :class="loading ? 'text-secondary cursor-not-allowed' : 'text-accent hover:bg-bg-tertiary'"
          :disabled="loading"
          @click="handleNewChat"
        >
          <Plus :size="13" />
        ...
        <div class="max-h-48 overflow-y-auto">
          <button
            ...
            :class="[
              ...
              loading && session.id !== currentSessionId ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer',
            ]"
            :disabled="loading && session.id !== currentSessionId"
            @click="handleSwitchSession(session.id)"
          >
            <div class="flex items-center gap-2 min-w-0 flex-1">
              <Check v-if="session.id === currentSessionId" :size="11" class="text-accent shrink-0" />
              <div v-else class="w-3 shrink-0" />
              <span class="truncate text-main">{{ session.name }}</span>
            </div>
            <span class="text-[10px] text-secondary shrink-0 ml-2">{{ getSessionMessageCount(session) }}</span>
          </button>
        </div>
        ...
        <button
          ...
          :class="loading ? 'text-secondary cursor-not-allowed' : 'text-red-500 hover:bg-bg-tertiary'"
          :disabled="loading"
