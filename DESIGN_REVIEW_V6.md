...
Le projet KickOffice est **solide architecturalement** avec une bonne separation frontend/backend, un sandbox SES pour l'execution de code, et une gestion correcte d'Office.onReady. Les principaux problemes se situent au niveau des **limites de tokens/contexte** (corrige dans la PR #164), des **violations DRY** entre les 4 hosts Office, et de quelques **lacunes UX** sur le streaming et la responsivite du taskpane.

| Severite | Count | Repartition |
| -------- | ----- | ----------- |
| BLOQUANT/CRITIQUE | 5 | Fonctionnalites: 2, Code: 1, Architecture: 1, UX: 1 |
| MAJEUR/IMPORTANT | 14 | Fonctionnalites: 5, Code: 5, Architecture: 2, UX: 2 |
| MINEUR/AMELIORATION | 16 | Fonctionnalites: 5, Code: 5, Architecture: 3, UX: 3 |
| **Total** | **35** | |
...
#### F-C1. Excel : `getItem(sheetName)` sans try/catch — crash si feuille inexistante

**Fichiers**:
- `frontend/src/utils/excelTools.ts` — lignes 1010-1012 (protectWorksheet)
- `frontend/src/utils/excelTools.ts` — ligne 1078 (setNamedRange)

**Probleme**: `context.workbook.worksheets.getItem(sheetName)` est appele sans protection. Si l'utilisateur ou le LLM fournit un nom de feuille inexistant, l'erreur propage sans etre catchee et casse la boucle agent.

**Impact**: Crash silencieux du tool call, l'agent ne recoit aucun message d'erreur exploitable.

**Fix**: Wrapper dans try/catch avec message d'erreur explicite pour le LLM.

---

#### F-C2. PowerPoint : `proposeShapeTextRevision` description trompeuse vs implementation

**Fichier**: `frontend/src/utils/powerpointTools.ts` — lignes 359-450

**Probleme**: La description du tool promet un "word-level diff" avec "preservation du formatting", mais l'implementation fait un `textRange.text = revisedText` (ligne 428) — remplacement total sans preservation du formatting par run.

**Impact**: Le LLM croit pouvoir faire du formatting granulaire alors qu'il perd tout le formatting existant. Particulierement problematique pour les presentations avec du formatting riche.

**Fix**: Soit implementer le diff par run, soit corriger la description pour refleter le comportement reel.
...
#### C-C1. Upload route : mammoth sans try/catch

**Fichier**: `backend/src/routes/upload.js` — lignes 70-71
...
```
Aucun try/catch autour de l'extraction DOCX. Si le fichier est corrompu, l'erreur propage sans reponse HTTP propre.

**Impact**: Crash du endpoint upload, erreur 500 non descriptive.

**Fix**: Wrapper dans try/catch comme c'est fait pour PDF (lignes 54-63).
...
#### C-M3. Double logging des erreurs dans le backend

**Fichier**: `backend/src/routes/chat.js` — lignes 141-142, 169-170, 280-281

**Probleme**:
```javascript
systemLog('ERROR', 'POST /api/chat stream error', streamError)
systemLog('ERROR', 'Stream error:', streamError)  // DOUBLON
```
Chaque erreur est loguee 2 fois avec des messages legerement differents.
...
#### C-M5. Messages d'erreur backend non localises

**Fichiers**: Tous les fichiers dans `backend/src/routes/*.js`

**Probleme**: Les messages d'erreur sont en anglais uniquement alors que le frontend supporte FR/EN via i18n.

**Exemples**:
- `chat.js:63`: `'The AI service returned an error. Please try again later.'`
- `upload.js:114`: `'Unsupported file type: ...'`

**Fix**: Renvoyer des codes d'erreur plutot que des messages, laisser le frontend traduire.
...
#### A-C1. Pas de registre centralise de codes d'erreur

**Scope**: Tout le projet

**Probleme**: Les erreurs sont gerees de maniere ad hoc :
- HTTP status codes (400, 401, 500, 502, 504) dans le backend
- Resultats d'outils custom (`success: false`)
- Messages d'erreur en texte libre
- Erreurs categorizees par inspection de message dans `backend.ts:36-69`

**Impact**: Impossible de mapper systematiquement erreur backend → message utilisateur traduit. Fragile si les messages d'erreur changent.

**Fix**: Creer un enum de codes d'erreur partage (`ERROR_CODES`) et retourner toujours `{ code: 'SHEET_NOT_FOUND', detail: '...' }`.
...
#### U-C1. Pas de streaming visible dans la boucle agent

**Fichier**: `frontend/src/composables/useAgentLoop.ts`

**Probleme**: En mode agent (avec tool calls), l'utilisateur attend 10-30 secondes avec un simple indicateur pulse sans voir le texte apparaitre progressivement. Le streaming existe pour la reponse finale mais pas pendant la phase de reflexion/outils.

**Impact**: L'utilisateur pense que l'application est bloquee.
...
### Priorite 1 — Bloquants (a faire maintenant)

| ID | Description | Fichier | Effort |
|----|-------------|---------|--------|
| F-C1 | try/catch sur `getItem(sheetName)` Excel | excelTools.ts | 15 min |
| F-C2 | Corriger description `proposeShapeTextRevision` | powerpointTools.ts | 10 min |
| C-C1 | try/catch mammoth dans upload.js | upload.js | 5 min |
| A-C1 | Enum codes d'erreur partage | Nouveau fichier | 2h |
| U-C1 | Streaming visible en boucle agent | useAgentLoop.ts | 4h |
...
| C-M3 | Supprimer double logging | chat.js | 15 min |
| C-M5 | Codes d'erreur au lieu de messages backend | routes/*.js | 2h |
